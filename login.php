...
require_once 'Google/Client.php';
 ...
 $client_id = '324056351916-5l4v4guuf942ihu222ugp35qoevatb48.apps.googleusercontent.com';
 $client_secret = 'd3n433IQhZaPCDSBhRJJk0C7';
 $redirect_uri = 'http://enttw.hopto.org/enlMS/login.php';
...
<form id="login" action="none">
    ...
    <h1 id="ff-proof" class="ribbon">ENL.tw</h1>

    <fieldset id="inputs">
    <input id="username" type="text"  value="訊息：請使用Google+帳號進行驗證" />
    <?php if (isset($authUrl)): ?>
		<a class='login' href='<?php echo $authUrl; ?>'  id="submit">Login</a>
    <?php else: ?>
		<a class='logout' href='?logout'  id="submit">Logout</a>
    <?php endif ?>
    </fieldset>
  ...
  <?php 
	if (isset($token_data)){
		...
		//check admin
		$SList = $db->query("SELECT * FROM `msadmin` WHERE `Email` LIKE '".$token_data['payload']['email']."'");
		$SList->setFetchMode(PDO::FETCH_ASSOC);
		$Sresult_arr = $SList->fetchAll();
		if(count($Sresult_arr)>=1)
		{
			$_SESSION["Codename"] = $Sresult_arr[0]['Codename'];
			$_SESSION["MSLevel"] = $Sresult_arr[0]['MSLevel'];
			$_SESSION["Location"] = $Sresult_arr[0]['Location'];
			$_SESSION["Email"] = $Sresult_arr[0]['Email'];
			echo "<script> alert('Welcome！". $Sresult_arr[0]['Codename']." 大大');location.href='index.php?page=agentList&Location=".$Sresult_arr[0]['Location']."';</script>";
		}else{
